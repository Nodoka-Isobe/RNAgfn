...
def main(args):
    ...
    G_vi = fc.pf()
    # --- 【修正】 pf_eval_energy の呼び出し ---
    # C言語側で文字列を操作・参照するため、変更可能なバッファを作成して渡します。
    # 1. sequence: 配列データ
    ...
    for it in (pbar := tqdm(range(args.n_iterations), dynamic_ncols=True)):
        ...
        #修正箇所ここから---
        # 【現在の logZ の値を取得】
        # tensor から Pythonの float に変換します
        current_logZ = gflownet.logZ.item()

        # --- 変更点1: 定期的なログ出力に logZ を追加 ---
        if it % 50 == 0:
        ...
        with torch.no_grad():
            # gflownet.logz_parameters() はリストを返すジェネレータ想定
            for param in gflownet.logz_parameters():
                # clamp_ (インプレース操作) で最小値を ref_logZ_value に固定
                param.data.clamp_(min=ref_logZ_value)

        # 修正箇所ここから---
        # --- 変更点2: プログレスバーの横に logZ と Ref を常時表示 ---
        # 常に更新後の値を確認したい場合は、再度取得しても良いですが、
        # ここでは clamp 前の値(または直後の値)が表示されます
        pbar.set_postfix({
            "loss": f"{loss.item():.4f}", 
            "logZ": f"{current_logZ:.4f}", 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lr_rnd", type=float, default=5e-2, help="Learning rate for the RND module")
    parser.add_argument("--rnd_reward_scale", type=float, default=3.0, help="Reward scale for RND")
    ...
    parser.add_argument("--sequence", type=str, default="UGCGAUCCGCGCUUCGGA", help="RNA sequence string")
