...
from gfn.gym import RNAStackingEnv, RNAStackingEnv_Re
...
def print_structure_frequencies(
    ...
    print("T =", env.temperature)
    print("RT =", RT)
    print("E =", energy)
    print("score =", np.exp(-energy / RT))
...
def main(args):
    ...
    env = RNAStackingEnv(seq, stacks, device=device, temperature = T)
    # preprocessor = KHotPreprocessor(height=4, ndim=len(seq))
    preprocessor = KHotPreprocessor(height=4, ndim=len(seq))  # スタックを形成しうるペアに限定
    ...
    logZ = nn.Parameter(
        torch.tensor((-G / RT), dtype=torch.float32, device=device)
    )
    ...
    for it in (pbar := tqdm(range(args.n_iterations), dynamic_ncols=True)):
        if(it > 10000) and (it < (args.n_iterations - 10000)):
            env.temperature = max(
                T_min,
                T * (T_decay ** (it - 10000))
            )
            ...
            with torch.no_grad():
                gflownet.logZ.fill_((-G / RT))

        if(it == (args.n_iterations - 10000)):
            optimizer.add_param_group({"params": gflownet.logz_parameters(), "lr": args.lr_logz})
        ...
        optimizer.step()
        # if it % 100 == 0:
        #     check_result = env.run_consistency_checks(
        #         trajectories.states,
        #         max_errors=100,
        #         max_to_print=20,
        ...
        #     )
            ...
            # print("error count:", len(check_result["errors"]))
            # print("collision count:", len(check_result["collisions"]))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lr_logz", type=float, default=1e-2, help="Learning rate for the logZ parameter")
    ...
    #焼なまし法用
    parser.add_argument("--T0", type=float, default=500.0, help="Initial temperature")
