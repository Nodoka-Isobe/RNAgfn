...
PUBLIC float  energy_of_struct(const char *string, const short *structure);
...
void print_pair_table(short *table);

int is_pseudoknot_boundary(int i, int j, short *ptable);

int calc_dp_pseudoknot_energy(int start, int end, short *ptable, const char *string);
...
float energy_of_struct(const char *string, const short *structure)
{
  int   energy;
  short *ss, *ss1;
  int i;
  ...
  pair_table = (short *) calloc(length + 2, sizeof(short));
  ...
  if (!pair_table) {
    ...
    // インデックス0に長さを格納
    pair_table[0] = (short) length;

    // 入力配列を1つずらしてコピー
    // structure[0] -> pair_table[1]
    // structure[1] -> pair_table[2] ...
    for (i = 0; i < length; i++) {
        pair_table[i + 1] = structure[i];
    }

  // print_pair_table(pair_table);
...
void print_pair_table(short *table) {
    if (table == NULL) {
        printf("Error: pair_table is NULL.\n");
        return;
    }

    int length = table[0]; // 0番目に配列長が入っている前提
    // printf("--- Pair Table (Length: %d) ---\n", length);
    // printf("Idx : Pair  : Visual\n");

    // ドットブラケット文字を再構築するためのバッファ
    char *db_str = (char *)malloc(sizeof(char) * (length + 1));

    for (int i = 1; i <= length; i++) {
        int pair_idx = table[i];
        char c;
...
int energy_of_struct_pt(const char *string, short * ptable, short *s, short *s1) {
  ...
  if (eos_debug>0) printf("External loop : %5d\n", energy);
  ...
  for (i = 1; i <= length; i++) {
    ...
    // 1. ペアを作っていない、または「閉じカッコ ')'」の場合はスキップ
    // ※元のコードは閉じカッコ(ptable[i] < i)のチェックが抜けているため、
    //   i=18のときに j=9 に戻って無限ループや不正計算を起こしている可能性があります。
    if (pair_table[i] <= i) continue;

    // 2. シュードノット境界かチェック
    if (is_pseudoknot_boundary(i, pair_table[i], pair_table)) {
        
        // A. シュードノットエネルギーを計算（ここですべての交差ペアが処理される）
        // int pk_energy = calc_dp_pseudoknot_energy(i, pair_table[i], pair_table, string);
        // energy += pk_energy;
        
        // B. 【重要】シュードノット領域の「本当の終わり」を探してジャンプする
        // 単に pair_table[i] まで飛ぶだけでは不十分です（交差相手がもっと後ろにいるため）。
        int current_end = pair_table[i];
        int max_end = current_end;
        
        // i から current_end までの間にあるペアが、範囲外(> current_end)に伸びていないか確認
        for (int k = i + 1; k < current_end; k++) {
            if (pair_table[k] > max_end) {
                max_end = pair_table[k];
            }
        }
        int pk_energy = calc_dp_pseudoknot_energy(i, max_end, pair_table, string);
        energy += pk_energy;
        // printf("Pseudoknot from %d to %d: Energy = %d\n", i, max_end, pk_energy);

        // C. ループカウンタ i をシュードノットの末端まで進める
        // これにより、内部のペア（8-19など）やヘアピンが二重に計算されるのを防ぐ
        i = max_end; 

    } else {
        // 3. 通常の構造（シュードノットではない）
        energy += stack_energy(i, string);
        // printf("Regular pair (%d, %d): Energy = %d\n", i, pair_table[i], stack_energy(i, string));
        // ペアの相手までジャンプ
        i = pair_table[i];
    }
  }
...
PRIVATE int stack_energy(int i, const char *string)
{
  ...
  j=pair_table[i];
  if (is_pseudoknot_boundary(i, j, pair_table)) {
        // fprintf(stderr, "WARNING: pseudoknot detected between bases %d and %d\n", i, j);
        return calc_dp_pseudoknot_energy(i, j, pair_table, string);
    }
  type = pair[S[i]][S[j]];
  if (type==0) {
    type=7;
  ...
  while (p<q) { /* process all stacks and interior loops */
    ...
    energy += ee;
    // printf("Debug: Adding energy %d from loop (%d,%d)-(%d,%d)\n", ee, i, j, p, q);
  ...
  if (p>q) {                       /* hair pin */
    if (SAME_STRAND(i,j))
      ...
      ee = ML_Energy(cut_in_loop(i), 1);
    energy += ee;
    // printf("Debug: Adding hairpin energy %d from loop (%d,%d)\n", ee, i, j);
    if (eos_debug>0)
...
// ---------------------------------------------------------
// シュードノット境界判定関数
// 領域 [i, j] の内部にある塩基 k が、領域外とペアを作っているかチェック
// ---------------------------------------------------------
int is_pseudoknot_boundary(int i, int j, short *ptable) {
    int k;
    for (k = i + 1; k < j; k++) {
        // k がペアを持っていて、その相手 ptable[k] が
        // i より小さい（左側にはみ出している） または
        // j より大きい（右側にはみ出している）場合
        if (ptable[k] != 0) {
            if (ptable[k] < i || ptable[k] > j) {
                return 1; // 交差あり（シュードノット）
            }
        }
    }
    return 0; // 交差なし（通常の入れ子構造）
}

int calc_dp_pseudoknot_energy(int start, int end, short *ptable, const char *string) {
    int energy = 0;
    int i, k;
    int length = ptable[0]; // ptableの0番目に長さが入っている前提

    // ----------------------------------------------------------------
    // 【修正箇所】開始ペナルティの二重計上防止ロジック
    // 「自分より左側(5'側)から始まって、自分と交差しているペア」があるか探す
    // ----------------------------------------------------------------
    int is_leftmost = 1; // デフォルトは「自分が一番左（代表）」
    ...
    for (k = 1; k < start; k++) {
        ...
        if (ptable[k] > k) {
            int l = ptable[k];
            // 交差判定: k < start < l < end
            // (kはループ条件で start未満 と確定しているので、lの位置だけ確認)
            if (l > start && l < end) {
                is_leftmost = 0; // 左側に交差相手がいるので、自分は代表ではない
                break;
            }
        }
    }

    // 自分が一番左のペア（代表）である場合のみ、開始ペナルティを加算
    if (is_leftmost) {
        energy += PK_penalty_external_37;
        // printf("Debug: Adding external pseudoknot penalty %d (Applied once for cluster)\n", PK_penalty_external_37);
    } else {
        // printf("Debug: Skipping external penalty (Already applied by crossing pair to the left)\n");
    }
    // ----------------------------------------------------------------
    ...
    for (i = start; i <= end; i++) {
        ...
        // A. 非対合塩基 (Unpaired bases)
        if (ptable[i] == 0) {
            energy += PK_penalty_unpaired_37;
            // printf("Debug: Adding unpaired penalty %d at position %d\n", PK_penalty_unpaired_37, i);
            continue;
        }
        ...
        if (i < ptable[i]) {
            ...
            // B. バンド開始ペナルティ (Band Initiation)
            // 直前がペアでない、または範囲外なら新しいバンド
            int prev_i = i - 1;
            int prev_j = j + 1;
            int is_new_band = 0;

            if (prev_i < start || prev_j > end) {
                is_new_band = 1; 
            } else if (ptable[prev_i] != prev_j) {
                is_new_band = 1; 
            }

            if (is_new_band) {
                energy += PK_penalty_band_37;
                // printf("Debug: Adding band initiation penalty %d at pair (%d,%d)\n", PK_penalty_band_37, i, j);
            }

            // C. 内側の構造（Stack / Internal Loop）を探す
            int p = i + 1;
            while (p < j && ptable[p] == 0) p++; 

            if (p < j && ptable[p] != 0) {
                int q = ptable[p];

                // 条件: (p, q) が (i, j) の内側にあること
                if (q < j && p < q) {
                    
                    int n1 = p - i - 1; 
                    int n2 = j - q - 1; 

                    // 右側のギャップ(q...j)の中身がすべてUnpairedか確認
                    int is_valid_loop = 1;
                    for (int k = q + 1; k < j; k++) {
                        if (ptable[k] != 0) { is_valid_loop = 0; break; }
                    }

                    if (is_valid_loop) {
                        int type = pair[S[i]][S[j]];
                        int type_2 = pair[S[q]][S[p]]; 
                        if (type == 0) type = 7;
                        if (type_2 == 0) type_2 = 7;

                        int raw_energy = LoopEnergy(n1, n2, type, type_2, S1[i+1], S1[j-1], S1[p-1], S1[q+1]);

                        // 乗法的ペナルティの適用 (×0.89 相当)
                        int final_loop_energy = (raw_energy * (int)PK_mult_stacked_pair) / 100;
                        energy += final_loop_energy;
                        
                        // printf("Debug: Adding loop energy %d from pseudoknot loop (%d,%d)-(%d,%d)\n", 
                              //  final_loop_energy, i, j, p, q);

                        // 【以前の修正】ここはコメントアウトのままにする（二重計上による安定化バグを防ぐため）
                        // energy -= (n1 + n2) * PK_penalty_unpaired_37;
                    }
                }
            }
        }
    }
    // printf("Debug: Total pseudoknot energy from (%d,%d) = %d\n", start, end, energy);
    return (float)energy;
}
/*---------------------------------------------------------------------------*/
