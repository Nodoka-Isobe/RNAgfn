...
class Sampler:
    ...
    def sample_actions(
        ...
        **policy_kwargs: Any,
        ...
        print(valid_actions_tensor)

        for i, a in enumerate(valid_actions_tensor):
            print(
                f"traj={i}, action={a.item()}, prob={dist.probs[i, a].item():.5f}"
            )
    ...
    def sample_trajectories(  # noqa: C901
        ...
        **policy_kwargs: Any,
        ...
        """
        # print("enter sample_trajectories")
        ...
        while not all(dones):
            ...
            # 👇 【修正追加】バックワードサンプリングの時だけ、環境のマスクを強制適用する
            if policy_estimator.is_backward:
                # 1. 現在の states のマスクを更新する
                env.update_masks(states[step_mask])
                
                # 2. 環境が計算したバックワードマスクを取得 (形状: [有効バッチ数, n_actions - 1])
                b_mask = states[step_mask].backward_masks
                
                # ⚠️ 環境側は eos_action (231) を除いたサイズでマスクを作っているので、
                # ポリシーの logits (サイズ 232) と合わせるために eos_action の位置を False (禁止) にする
                # (もし s0 に戻るための eos 選択肢を常に禁止したい場合)
                extended_mask = torch.zeros_like(dist.logits, dtype=torch.bool)
                extended_mask[:, :b_mask.shape[1]] = b_mask
                
                # 3. マスクされていない (Falseの) 部分のロジットを -inf で埋める
                dist.logits = dist.logits.masked_fill(~extended_mask, float("-inf"))
                
                # 4. 確率（probs）もロジットに合わせて再計算
                dist.probs = torch.softmax(dist.logits, dim=-1)
            ...
            if policy_estimator.is_backward:
                ...
                # print("sampled:", valid_actions_tensor)
                # print("step_mask:", step_mask)
                # print("chosen actions =", actions.tensor.squeeze())
...
class StackLocalSearchSampler(LocalSearchSampler):
    def __init__(
        self,
        pf_estimator: Estimator,
        pb_estimator: Estimator,
        ...
        Args:
            ...
            pb_estimator: The backward policy estimator for backtracking trajectories.
        """
        super().__init__(pf_estimator, pb_estimator)
        self.backward_sampler = Sampler(pb_estimator)
...
#----------------------------------------------------------------------------------------------
        ...
        with torch.no_grad():
            all_states = prev_trajectories.states.tensor  # [T, B, seq_len]
            term_idx = prev_trajectories.terminating_idx  # [B]
            B = all_states.shape[1]

            # 各終点のスタッキング領域数を取得
            final_states = torch.stack([all_states[term_idx[b], b] for b in range(B)])
            # print("prev final states:", final_states)
            final_counts = env.count_stacking_regions(final_states)  # [B]

            # 出力を初期化
            junction_states = []
            n_prevs_list = []  # <-- [修正] n_prevs を保存するリスト
            
            # 各バッチで処理
            for b in range(B):
                target_count = final_counts[b] - 1
                found_state = None
                found_t = 0  # <-- [修正] 見つかったインデックスを保存
                
                # 終点から逆順にたどって探す
                for t in range(term_idx[b], -1, -1):
                    s_t = all_states[t, b]
                    count_t = env.count_stacking_regions(s_t.unsqueeze(0))[0]
                    if count_t == target_count:
                        found_state = s_t
                        found_t = t  # <-- [修正] インデックスを記録
                        break

                # 見つからなければ最初の状態を使う
                if found_state is None:
                    found_state = all_states[0, b]
                    found_t = 0  # <-- [修正] 最初のインデックスは 0

                junction_states.append(found_state)
                n_prevs_list.append(found_t)  # <-- [修正] リストに追加

            junction_states_tsr = torch.stack(junction_states, dim=0)  # [B, seq_len]
            
            # <-- [修正] リストをテンソルに変換して n_prevs を定義
            n_prevs = torch.tensor(n_prevs_list, device=all_states.device, dtype=torch.long)
            
        # print("junction states:", junction_states)
        #--------------------------------------------------------------------------------
        recon_trajectories = super().sample_trajectories(
            env,
            states=env.states_from_tensor(junction_states_tsr),
        ...
        # print("recon trajectories:", recon_trajectories)
