...
class DiscreteActions(Actions):
    """Actions for discrete environments.

    Attributes:
        tensor: Tensor of shape (*batch_shape, 1) representing a batch of actions.
    """

    action_shape = (1,)
    dummy_action = torch.tensor([-1], dtype=torch.long)
    exit_action = torch.tensor([-1], dtype=torch.long) # 環境によって上書きされる場合がありますが、初期値として定義が必要です